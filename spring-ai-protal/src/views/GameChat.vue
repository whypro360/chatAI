    ...
    <div class="game-container">
      ...
      <div v-if="!isGameStarted" class="game-start">
        ...
        <div class="input-area">
          <textarea
            v-model="angerReason"
            placeholder="你扮演的是长辈，请输入幼儿不开心得原因（可选）..."
            rows="3"
          ></textarea>
...
const startGame = async () => {
  ...
  forgiveness.value = 0  // 重置开心值
  
  // 发送开始游戏请求
  const startPrompt = angerReason.value 
    ? `开始游戏，孙子不开心的原因：${angerReason.value}`
    : '开始游戏'
  ...
  try {
    ...
    while (true) {
      try {
        ...
        // 尝试从回复中提取开心值
        const forgivenessMatch = accumulatedContent.match(/开心值[:：]\s*(\d+)/i)
        if (forgivenessMatch) {
    ...
    if (currentRound.value >= MAX_ROUNDS) {
      isGameOver.value = true
      if (forgiveness.value >= 100) {
        gameResult.value = '恭喜你！在最后一轮成功哄好了孙子！💕'
      } else {
        gameResult.value = `游戏结束：对话轮次已达上限(${MAX_ROUNDS}轮)，当前开心值为${forgiveness.value}，很遗憾没能完全哄好孙子`
      }
    }
    // 检查是否游戏结束
